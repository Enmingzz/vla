...
def main():
    ...
    for h in [5] + [h for h in args.horizons if h != 5]:
        for seed in args.seeds:
            subprocess.run([sys.executable, "-m", "frequency_vla.evaluator", "--openpi-dir", args.openpi_dir,
                "--mode", args.mode, "--suite", args.suite, "--horizon", str(h), "--required-horizon", str(max(args.horizons)),
                "--seed", str(seed), "--host", args.host, "--port", str(args.port), "--results-dir", args.results_dir], check=True)

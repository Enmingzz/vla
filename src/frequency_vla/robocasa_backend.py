"""RoboCasa port of the archived temporal OPSD backend.

The optimization and shared-tail flow protocol are unchanged. Differences are
12 physical action dimensions and a separate RoboCasa configuration validator.
Keeping this module separate preserves historical LIBERO source checksums.
"""
import copy
import json
import logging
...
from .robocasa_protocol import align_teacher_latent, validate_training_config
...
class TemporalOPSD:
    def __init__(self, policy, metadata, config, results_dir, checkpoint_root, source_checkpoint):
        ...
        self.sources = {name: file_digest(Path(__file__).parent / name)
                        for name in ["robocasa_backend.py", "opsd_flow.py", "opsd_protocol.py", "robocasa_protocol.py"]}
        ...
        def update(master, opt_state, frozen, observation, latent, timestep, targets, mask):
            ...
            def loss_fn(params):
                prediction = field(params, frozen, observation, latent, timestep)[:, :20, :12]
                squared = jnp.square(prediction.astype(jnp.float32) - targets.astype(jnp.float32))
                loss = 0.5 * jnp.sum(squared * mask[..., None]) / (jnp.sum(mask) * 12)
                per_offset = jnp.sum((squared * mask[..., None]).reshape((-1, 4, 5, 12)), axis=(0, 2, 3))
                per_offset /= jnp.maximum(1, jnp.sum(mask.reshape((-1, 4, 5)), axis=(0, 2)) * 12)
    ...
    def learn(self, future_observations, control):
        ...
        if pending["diagnostic"]:
            ...
            forward = np.asarray(self.field(self.master, self.frozen,
                pending["observation"], student_z, timestep), dtype=np.float32)[:, :20, :12]

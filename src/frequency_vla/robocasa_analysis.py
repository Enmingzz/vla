...
def paired(rows_a,rows_b):
    ...
    for k in a:
        fields = ['initial_state_sha256','initial_observation_sha256',
                  'environment_metadata_sha256','config_sha256','prediction_horizon','flow_steps','renderer']
        if all('initial_xml_comparison_sha256' in r for r in (a[k],b[k])):
            fields += ['initial_xml_comparison_sha256','paired_catalog_identity_sha256']
        else:
            fields += ['initial_xml_sha256']
        for field in fields:
...
def analyze(root):
    ...
    with (root/'analysis.lock').open('a') as lock:
        ...
        summaries,rows,progress,all_rows = {},{},{},{}
        for folder,condition in CONDITIONS:
            path = root/folder/'aggregated'/(condition+'.json')
            records = [json.loads(line) for file in sorted((root/folder/'raw'/condition).glob('*.jsonl'))
                       for line in file.read_text().splitlines() if line]
            all_rows[condition] = records
            progress[condition] = {'completed_episodes':len(records),
                'successes':sum(r['success'] for r in records),'complete':path.exists()}
            if path.exists():
        ...
        # Interruption depends on task order, so partial samples are descriptive
        ...
        for name,a,b in [('replanning','original_h5','original_h20'),
            ...
            if a in rows and b in rows:
                continue
            key = lambda r:(r['task_id'],r['episode_index'],r['episode_seed'])
            keys = {key(r) for r in all_rows[a]} & {key(r) for r in all_rows[b]}
            if keys:
                aa,bb = [[r for r in all_rows[c] if key(r) in keys] for c in (a,b)]
                paired(aa,bb)  # Enforce the same exact initial states/configuration.
                partial_comparisons[name] = {'condition_a':a,'condition_b':b,
                    'matched_episodes':len(keys),'successes_a':sum(r['success'] for r in aa),
                    'successes_b':sum(r['success'] for r in bb),'descriptive_only':True}
        write_json(out/'progress.json',{'conditions':progress,'partial_comparisons':partial_comparisons})
        ...
        for _,condition in CONDITIONS:
            if condition not in summaries:
                r = progress[condition]
                status = ('Incomplete: {}/{} completed episodes succeeded'.format(
                    r['successes'],r['completed_episodes']) if r['completed_episodes'] else 'Not started')
        ...
        for name,c in partial_comparisons.items():
            lines += ['', 'Incomplete {} comparison, restricted to the same {} completed episodes: {} {}/{}; {} {}/{}. '
                'This task-order-dependent subset is descriptive and is not the full ten-task result.'.format(
                    name,c['matched_episodes'],c['condition_a'],c['successes_a'],c['matched_episodes'],
                    c['condition_b'],c['successes_b'],c['matched_episodes'])]
        ...
        if len(summaries)<4:
            lines += ['', 'Experiment incomplete: missing episodes are not scored as failures.']

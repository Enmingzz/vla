#!/usr/bin/env bash
# Add only the missing 400 H=5 episodes per checkpoint; retain cached outcomes.
set -euo pipefail
source "${FREQUENCY_PROJECT:-${SLURM_SUBMIT_DIR:-$PWD}}/scripts/env.sh"
cd "$FREQUENCY_PROJECT"
: "${RUN_RESULTS:?Set the prepared, fresh extension archive}"
...
env -u PYTHONPATH -u PYTHONHOME -u LD_LIBRARY_PATH "$LIBERO_VENV/bin/python" \
  -m frequency_vla.h5_extension verify --results-dir "$RUN_RESULTS"
if [[ -e "$RUN_RESULTS/provenance/frozen_comparison.json" ]]; then
  echo "Use a fresh result directory; preserve previous attempts." >&2
  exit 2
fi
...
PY
env -u PYTHONPATH -u PYTHONHOME -u LD_LIBRARY_PATH "$LIBERO_VENV/bin/python" \
  -m frequency_vla.h5_extension run --results-dir "$RUN_RESULTS" --port "$POLICY_PORT"

...
修复后的作业 `60042686` 已成功完成 H=5/15/20 共 300 个 smoke 回合及
1 个独立诊断回合：单卡运行 **25 分 21 秒**，主内存峰值 **27.45 GB**，
申请 40 GB；结束后已自动释放。包含上述两次失败尝试，这次 follow-up
总共消耗 **55 分 26 秒单卡时间**。这也说明需要把失败时间计入资源效率。

随后授权的 H=20 训练试验使用 1 张 H100、8 CPU、48 GiB 内存。首次数值
诊断 `60044854` 在 **4 分 55 秒**后退出；正式作业 `60045256` 用
**22 分 12 秒**完成诊断、100 局训练前评测、100 步训练、checkpoint 导出和
100 局训练后评测。该阶段含失败诊断共 **27 分 07 秒单卡时间**；与上面的
频率 follow-up 合计 **1 小时 22 分 33 秒**。显存以外的主内存峰值为
**42.68 GiB**，因此训练／导出阶段保留 48 GiB 申请有实测依据。
结束后 GPU 已释放。详见 `results/opsd_h20_100/provenance/resource_accounting.json`。
